...
const byte Relay_0 = 36;
const byte Relay_1 = 37;
const byte Relay_2 = 38;
const byte Relay_3 = 39;
const byte Relay_4 = 40;
const byte Relay_5 = 41;
const byte Relay_6 = 42;
const byte Relay_7 = 43;
const byte Relay_8 = 44;
const byte Relay_9 = 45;
const byte Relay_A = 46;
const byte Relay_B = 47;
    ...
    case 2:   // Trap 1 cooling
      {
        switch (submode)
        {
          case 0:   // lift 1 N2 cylinder and start cooling it
            {
              setValve(TRAP_2_LIFT, 1);     // Raise 1 N2 cylinder
              previousMillis = millis();
              submode++;
              break;
            }
    ...
    case 4:   // Measure CO2 (a lot of stuff happens here)
      {
        switch (submode)
        {
          case 0:   // Connect valves for CO2 measurements, lift 2 trap, start cooling trap 2
            {
              setValve(Relay_2, 0);
              setValve(Relay_3, 0);
              setValve(Relay_4, 0);
          ...
          case 1:   // Wait for trap 2 to cool, release 1 N2 cylinder, start CO2 measurements
            {
              currentMillis = millis();
    ...
    case 5:   // Measure beta counts
      {
        switch (submode)
        {
          ...
          case 1:   // Wait for vacuum
            {
              if (analogRead(A_Pres_2) <= 4096)    // When vacuum is OK  // <<<<<<-------------------------------------    Here will be vacuum waiting variable, but later
              {
                ResetC14();
                submode++;
              }
          ...
          case 2:   // Prepare for trap heating
            {
              ...
              setValve(Relay_B, 1);   // Turn off vacuum pump
    ...
    case 6:   // Vent everything and prepare system for second measurement
      {
        switch (submode)
        {
          ...
          case 1:   // Vent C14 camera through water
            {
              currentMillis = millis();
              if (currentMillis - previousMillis >= 180000)  // When C14 camera is vented enough
              {
          ...
          case 2:   // vent traps with carrier gas
            {
              currentMillis = millis();
              if (currentMillis - previousMillis >= 120000)  // When traps is vented enough
              {
