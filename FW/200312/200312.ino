...
const byte TRAP_1_LIFT = 7;
const byte TRAP_1_HEAT = 6;
...
unsigned long CO2Time = 300000;   // CO2 measurement time

//************************************ Zeolite Config ************************************

unsigned int PreheatCO2 = 50;
unsigned int ReleaseCO2 = 160;
unsigned int FlushCO2 = 180;
unsigned int ZSet = 0;
unsigned int ZTemp = 0;
unsigned int ZDB = 2;

//************************************ SPI config ************************************
//SPISettings settingsA(2000000, MSBFIRST, SPI_MODE1);
SPISettings max6675(2100000, MSBFIRST, SPI_MODE1);
...
void setup()
{
  ...
  pinMode(ZHeat, OUTPUT);   // Zeolite trap heater
  pinMode(ZFan, OUTPUT);   // Zeolite trap fan
  ...
  pinMode(max6675_ss, OUTPUT);   // Slave select for MAX6675
  ...
  SPI.begin(10);
  // Set clock divider on pin 10 to 20 (2,1 MHz clock)
  SPI.setClockDivider(10, 40);
...
void loop()
{
  ...
  switch (mode)
  {
          ...
          case 0:   // lift 1 N2 cylinder and start cooling it
            {
              ...
              setValve(Relay_2, 0);         // Close valve 2-7 (for system hermetisation)
              setValve(Relay_3, 0);
              setValve(Relay_4, 0);
              setValve(Relay_5, 0);
              setValve(Relay_6, 0);
          ...
          case 1:   // Wait for trap to cool
            {
              ...
              {
                setValve(Relay_2, 1);     // Open valve 2 for sample to go in
                mode_req = 3;
                update = true;
              };
              break;
    ...
    case 3:   // Sample going into trap
      {
        switch (submode)
        {
          ...
          case 1: // Wait for sample to accumulate in trap 1
            {
              currentMillis = millis();
              if (currentMillis - previousMillis >= SampleTime)  // when sampling done stop vacuum and flush with carrier gas
              {
                previousMillis = millis();
                submode++;
                setValve(Relay_B, 0);     // Relay B is vacuum pump
                setValve(Relay_0, 0);     // Relay 0 is vacuum valve

                setValve(Relay_1, 0);     // Switch sample to carrier gas
                setValve(Relay_6, 1);     // Open vent valve after trap 2
                setValve(Relay_7, 1);     // Isolate C14 camera
                setValve(Relay_9, 0);     // Open carrier gas main valve
          ...
          case 2: // Wait for sample to accumulate in trap 1
            {
              currentMillis = millis();
              if (currentMillis - previousMillis >= FlushTime)  // carrier gas flushing done
              {
                mode_req = 4;
                update = true;
              };
    ...
    case 4:   // Measure CO2 (a lot of stuff happens here)
      {
        switch (submode)
        {
          ...
          case 2:   // Wait 0.5S until N2 is down and start trap 1 heating
            {
              currentMillis = millis();
              if (currentMillis - previousMillis >= 3000)  // When N2 is down
              {
          ...
          case 5:   // Measure CO2 going throug column
            {
              currentMillis = millis();
              if (currentMillis - previousMillis >= CO2Time)  // When everything is gone (after 60 seconds)
              {
    ...
    case 5:   // Measure beta counts
      {
        switch (submode)
        {
          ...
          case 3:   // Wait for trap 2 to go down
            {
              currentMillis = millis();
              if (currentMillis - previousMillis >= 3000)  // When trap 2 is down
              {
    ...
    case 6:   // Vent everything and prepare system for second measurement
      {
        switch (submode)
        {
          ...
          case 1:   // Vent C14 camera through water
            {
              currentMillis = millis();
              if (currentMillis - previousMillis >= 60000) //180000  // When C14 camera is vented enough
              {
          ...
          case 2:   // vent traps with carrier gas
            {
              currentMillis = millis();
              if (currentMillis - previousMillis >= 30000) // 120000 // When traps is vented enough
              {
    ...
    */
    ...
    case 255:   // Manual mode
      {
        switch (submode)
        {
          case 0:   // Set required parameters for manual mode
            {
              StartCO2();
              submode++;
              break;
  ...
  if (valves & 0x08)    // If heating process is going (bit 4 is set)
  {

    if ((valves & 0x08) == HeatNumber)    // If nothing in trap heat is changed
    {
    ...
    {
      HeatNumber = (valves & 0x08);   // Set what traps is heating
  ...
  if (timeForData)
  {
    switch (InfoIndex)
    {
      ...
      case 1:
        {
          Zeolite();      // Update zeolite temperatures, set required values.
          printCO2();
          break;
        }
  ...
  if (digitalRead(Knopke) == 0)
  {
    if (ButtonStatus)     // If button was pressed earlier
    {
      ...
      if (ButtonNowMillis - ButtonPushMillis >= 3000)  // button is pressed longer than 3 seconds
      {
        ...
        /*  DO NOT DELETE! Useful code for later
...
void SYSinit (void)
{
  digitalWrite(ZHeat, LOW);
  digitalWrite(ZFan, LOW);
...
//---------------------------------- Zeolite update functionn --------------------------------------------
void Zeolite()
{

  char ZState = (valves & 0x03);
  GetZTemp();
  //unsigned int ZTemp;
  switch (ZState)
  {
  ...
  if (ZTemp < (ZSet - ZDB))
  {
    digitalWrite(ZHeat, HIGH);  // Turn on heater
  }
  else if (ZTemp >= ZSet)
  {
    digitalWrite(ZHeat, LOW);
  ...
  if (ZTemp > (ZSet + ZDB))
  {
    digitalWrite(ZFan, HIGH);
  }
  else if (ZTemp <= ZSet)
  {
    digitalWrite(ZFan, LOW);    // Turn off fan
  };
...
void GetZTemp()
{
  byte val1 = SPI.transfer(10, 0x00, SPI_CONTINUE);
  byte val2 = SPI.transfer(10, 0x00);
  ZTemp = (val2 >> 6) | (val1 << 2);
  //Serial.println(ZTemp, DEC);
}
...
void parseData() {      // split the data into its parts
  ...
  while (completed == 0)
  {
    ...
    else if (strcmp(parsedString, "MTIME") == 0)     // CO2 measurement time setting
    {
      strtokIndx = strtok(NULL, ";"); // this continues where the previous call left off, find digit after = until ;
      tmpval = atoi(strtokIndx);      // Convert it to integer
      if ((tmpval <= 600) && (tmpval > 1)) // Check if command is OK
      {
        CO2Time = tmpval * 1000;
        //update = true;
        Serial.print("Set CO2 time: ");
        Serial.print(CO2Time / 1000);
        Serial.println(" seconds");
      } else
      {
        Serial.println("BAD CO2 time parameter < 1 s or > 600 s");
      };

      parsedString[0] = '\0';
    }   // End of "MTIME"

    else if (strcmp(parsedString, "PTEMP") == 0)     // CO2 measurement time setting
    {
      strtokIndx = strtok(NULL, ";"); // this continues where the previous call left off, find digit after = until ;
      tmpval = atoi(strtokIndx);      // Convert it to integer
      if ((tmpval <= 250) && (tmpval > 1)) // Check if command is OK
      {
        PreheatCO2 = tmpval;
        //update = true;
        Serial.print("Set trap preheat temperature: ");
        Serial.print(PreheatCO2);
        Serial.println(" C");
      } else
      {
        Serial.println("BAD preheat temperature < 1 C or > 250 C");
      };

      parsedString[0] = '\0';
    }   // End of "PTEMP"

    else if (strcmp(parsedString, "RTEMP") == 0)     // CO2 measurement time setting
    {
      strtokIndx = strtok(NULL, ";"); // this continues where the previous call left off, find digit after = until ;
      tmpval = atoi(strtokIndx);      // Convert it to integer
      if ((tmpval <= 250) && (tmpval > 1)) // Check if command is OK
      {
        ReleaseCO2 = tmpval;
        //update = true;
        Serial.print("Set trap CO2 release temperature: ");
        Serial.print(ReleaseCO2);
        Serial.println(" C");
      } else
      {
        Serial.println("BAD release temperature < 1 C or > 250 C");
      };

      parsedString[0] = '\0';
    }   // End of "RTEMP"

    else if (strcmp(parsedString, "FTEMP") == 0)     // CO2 measurement time setting
    {
      strtokIndx = strtok(NULL, ";"); // this continues where the previous call left off, find digit after = until ;
      tmpval = atoi(strtokIndx);      // Convert it to integer
      if ((tmpval <= 250) && (tmpval > 1)) // Check if command is OK
      {
        ReleaseCO2 = tmpval;
        //update = true;
        Serial.print("Set trap release temperature: ");
        Serial.print(ReleaseCO2);
        Serial.println(" C");
      } else
      {
...
void ResetC14 (void)
{
  digitalWrite(C14_skait_EN, HIGH);
  digitalWrite(C14_skait_A, HIGH);      // Reset gignal
  digitalWrite(C14_skait_B, HIGH);
  digitalWrite(C14_skait_C, HIGH);
  shittyDelay(1);                       // Shitty delay made of digitalRead (0) to dev/null
  digitalWrite(C14_skait_ADR, HIGH);    // Enable 74LS138P output (Reset select)
  shittyDelay(10);
  digitalWrite(C14_skait_ADR, LOW);     // Disable 74LS138P output (Reset select)
  // Copy counter data to buffers
  digitalWrite(C14_skait_WR, HIGH);     // Toggle data copy
  shittyDelay(1);
  digitalWrite(C14_skait_WR, LOW);
  digitalWrite(C14_skait_EN, LOW);
}
