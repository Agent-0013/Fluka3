          ...
          case 0:   // Start sample pump
            {
              ...
              setValve(Relay_5, 1);       // Start sample pump relay_5
          ...
          case 3: // Wait for sample time
            {
              ...
              {
                submode++;
                setValve(Relay_0, 1);   // Realy_0 = valve A ON (Open carrier gas)
                setValve(Relay_1, 1);   // Realy_1 = valve B ON (Open loop to CO2 measurement and 2 TRAP)
                setValve(Relay_5, 0);   // Realy_5 = Stop sampling pump
                mode_req = 2;
                update = true;
              }
              break;
    ...
    case 2:   // Release and Measure CO2
      {
        switch (submode)
        {
          case 0:   // Configure valves for heating
            {
              setValve(Relay_0, 1);   // Realy_0 = valve A ON (Open carrier gas)      // Should be open already, just in case started from this mode
              setValve(Relay_1, 1);   // Realy_1 = valve B ON (Open loop to CO2 measurement and 2 TRAP)
              previousMillis = millis();
