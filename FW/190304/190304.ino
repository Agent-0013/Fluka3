    ...
    case 1:   // Sample aquire started
      {
        switch (submode)
        {
          case 0:   // Lift the 1st N2 cylindeer
            {
          ...
          case 1:   // Wait for cylinder to get cold and turn on sample gas
            {
              currentMillis = millis();
              if (currentMillis - previousMillis >= 500)  // Delay 500ms for trap to get cold
              {
                setValve(Relay_0, 1);   // Realy_0 = valve A ON (let the sample gas in)
                submode++;
                previousMillis = millis();
              };
              break;
            }
          case 2: // Wait for sample time
            {
              currentMillis = millis();
              if (currentMillis - previousMillis >= SampleTime)  // Delay 60s for sample to aquire
              {
                submode++;
                setValve(Relay_0, 0);   // Realy_0 = valve A OFF (trap the sample gas)
                mode_req = 2;
                update = true;
              }
              break;
    ...
    case 2:   // Release and Measure CO2
      {
        switch (submode)
        {
          ...
          case 1:   // Wait for cylinder to get down
            {
              currentMillis = millis();
              if (currentMillis - previousMillis >= 100)  // Delay 100ms for trap to get down
              {
                setValve(Relay_1, 1);
                setValve(Relay_3, 1);
                setValve(Relay_5, 1);
          ...
          case 3:
            {
              currentMillis = millis();
              if (currentMillis - previousMillis >= HeatTime)  // Delay 1,5s for trap 1 to heat
              {
                submode++;
                setValve(TRAP_1_HEAT, 0);
                // Start CO2 counting function
              }
              break;
